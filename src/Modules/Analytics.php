...
final class Analytics extends \Kuetemeier\WordPress\PluginModule
{
    ...
    public function frontendInit()
    {
        ...
        if (!empty($this->getOption('header-code'))) {
            add_action('wp_head', array(&$this, 'callbackAddHeaderCode'), 0);
        }

        if (!empty($this->getOption('footer-code'))) {
            add_action('wp_footer', array(&$this, 'callbackAddFooterCode'), PHP_INT_MAX);
        }

        if (!empty($this->getOption('ga-tracking-id'))) {
            add_action('wp_head', array(&$this, 'callbackGoogleAnalyticsHeadCode'), 0);
        }
    }
    ...
    public function callbackAddHeaderCode()
    {
        echo do_shortcode($this->getOption('header-code'));
    }


    public function callbackAddFooterCode()
    {
        echo do_shortcode($this->getOption('footer-code'));
    }

    /**
     * Callback: Add Google Analytics Code to <head>.
     *
     * @see https://developers.google.com/analytics/devguides/collection/gtagjs/ip-anonymization
     */
    public function callbackGoogleAnalyticsHeadCode()
    {
